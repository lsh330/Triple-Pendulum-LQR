...
2. [라그랑주 역학](#2-라그랑주-역학)
3. [질량행렬](#3-질량행렬-mq)
4. [코리올리스 및 원심력 벡터](#4-코리올리스-및-원심력-벡터-cqq̇q̇)
5. [중력 벡터](#5-중력-벡터-gq)
...
$y_{cm,k}$가 코사인항을 포함하므로, 코드에서는 아래와 같은 `PE_code`를 내부 계산에 활용한다.

$$\text{PE\_code} = \sum_{k=1}^{3} gg_k \cos\phi_k$$

**부호 관계**: $V_{\text{physical}} = -\text{PE\_code}$

따라서 물리적 전체 에너지는:

$$E = T + V_{\text{physical}} = T - \text{PE\_code} = \text{KE} - \text{PE\_code}$$
<!-- source: control/swing_up/energy_computation.py:3-18 -->

Down 평형($\phi_k = 0$)에서 $\text{PE\_code} > 0$ (양수)이며 $V_{\text{physical}} < 0$.  
Up 평형($\phi_k = \pi$)에서 $\cos\phi_k = -1$이므로 $\text{PE\_code} < 0$이며 $V_{\text{physical}} > 0$.
...
각 평형점에서의 물리적 위치에너지 $V_{\text{physical}} = -\text{PE\_code}$:
...
$$B_u = M^{-1} \begin{bmatrix} 1 \\ 0 \\ 0 \\ 0 \end{bmatrix} = M^{-1} \text{의 첫 번째 열}$$
...
적응 스텝: $h_j = \varepsilon_{\text{mach}}^{1/3} \cdot \max(1, |{dq}_j|) \approx 6.055 \times 10^{-6}$
...
$$Q = \text{diag}(10, 100, 100, 100, 1, 10, 10, 10)$$
$$R = 0.01$$
...
목표 평형점에서 $\text{KE} = 0$이므로:

$$E^* = V_{\text{physical}}^* = -\text{PE\_code}^* = -(gg_1\cos\phi_1^* + gg_2\cos\phi_2^* + gg_3\cos\phi_3^*)$$
...
$$E = \underbrace{\frac{1}{2}\dot{\mathbf{q}}^T M(\mathbf{q})\dot{\mathbf{q}}}_{\text{KE}} - \underbrace{(gg_1\cos\phi_1 + gg_2\cos\phi_2 + gg_3\cos\phi_3)}_{\text{PE\_code}}$$
...
- **Swing-Up → LQR Catch**: $V < \rho_{\text{in}}$
- **LQR Catch → Swing-Up** (이탈): $V > \rho_{\text{out}}$
- **LQR Catch → Stabilized**: 편차 합계 $< 0.1$이 500 스텝 연속
...
$$\rho = \alpha \cdot \rho_{\max}, \quad \alpha = 0.8 \text{ (안전율)}$$
...
- 특이성 체크: $|\det M| < 10^{-12} \cdot (\max \text{diag})^4$
